...

{
  ...
  config.lib.monitors.mainMonitorName = lib.pipe config.monitors [
    builtins.attrNames 
    (names: builtins.filter (name: config.monitors.${name}.isMain) names)
    builtins.head
  ];
  config.lib.monitors.otherMonitorsNames = lib.pipe config.monitors [
    builtins.attrNames 
    (names: builtins.filter (name: !config.monitors.${name}.isMain) names)
  ];
