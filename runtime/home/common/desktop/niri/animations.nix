{config, nevik, ...}: {
  programs.niri.settings.animations = {
    ...
    window-close = {
      ...
      custom-shader = let
        mkGLColor = nevik.mkGLColor { inherit config; };
      in
        # glsl
        ''
          ...
          vec4 getFireColor(float v) {
              ...
              vec4 colors[5];
              colors[0] = ${mkGLColor "base00"} * 1.0;
              colors[1] = ${mkGLColor "base08"} * 1.0;
              colors[2] = ${mkGLColor "base09"} * 1.0;
              colors[3] = ${mkGLColor "base0A"} * 1.0;
              colors[4] = ${mkGLColor "base05"} * 1.0;
