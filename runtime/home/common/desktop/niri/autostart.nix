  ...
  niri-autostart = pkgs.writeShellApplication {
    ...
    text =
      ...
      + (lib.pipe config.monitors [
        builtins.attrNames
        (map (monitor: [
          "swww img --namespace background -o ${monitor} ${toString config.wallpaper.normal}"
          "sleep 0.2"
          "swww img --namespace backdrop -o ${monitor} ${toString config.wallpaper.blurred}"
          "sleep 0.2"
        ]))
...
wallpapers_path = "/home/${username}/snowflake/assets"
events_of_interest = [
    "Workspace changed",
    "Workspace focused",
    "Window opened",
...
def set_wallpaper_if_needed(active_workspace, init):
    ...
    wallpaper = os.path.join(wallpapers_path, f"{wallpaper_name}.png")
    blurred_wallpaper = os.path.join(wallpapers_path, f"{wallpaper_name}-blurred.png")
